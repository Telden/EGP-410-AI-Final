...
std::stack<Node*> AStarPathfinder::findPath(Node* pFrom, Node* pTo)
{
	//The overall path back to the target
	std::stack<Node*>path;
...
NodeRecord* AStarPathfinder::getSmallestCost(std::vector<NodeRecord*> vect, Node* to)
{
	...
	for (unsigned int i = 0; i < vect.size(); i++)
	{
		...
		fromPos = mpGraph->getNode(i)->getPosision();
